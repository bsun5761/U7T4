...
public class ArrayListAlgorithms
{
    ...
    public static void replaceWithCaps(ArrayList<String> wordList)
    {
        for(int i = 0; i< wordList.size(); i++)
        {
            ...
            {
                String high = current.toUpperCase();
                wordList.set(i, high);
            }
        }
    }
    ...
    public static boolean areIdentical(ArrayList<Integer> numList1, ArrayList<Integer> numList2)
    {
        ...
        if(numList1.size()==numList2.size())
        {
            ...
            {
                for(int k=0; k<numList2.size(); k++)
                {
                    if(numList1.get(i)==numList2.get(k))
                    {
                        same = true;
                    }
                    else
    ...
    public static void wackyVowels(ArrayList<String> wordList)
    {
        for(int i=0; i<wordList.size(); i++)
        {
            if(wordList.get(i).contains("a")||wordList.get(i).contains("e")||wordList.get(i).contains("o")||wordList.get(i).contains("i"))
            {
                wordList.remove(i);
            }
            if(wordList.get(i).contains("u")||wordList.get(i).contains("y"))
            {
